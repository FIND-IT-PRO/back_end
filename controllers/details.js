...
const getAllData = async (req, res) => {
  try {
    const data = await Detail.find({ status: "active" }).populate(
      "typeObjet_id contact_id"
    );
...
const getDataById = async (req, res) => {
  try {
    const data = await Detail.findById(req.params._id).populate(
      "typeObjet_id contact_id"
    );
    res.send(data).status(200);
  } catch (exception) {
...
const updateData = async (req, res) => {
  try {
    const data = await Detail.findByIdAndUpdate(req.params._id, req.body, {
      new: "true",
    });
    res.send(data).status(200);
  } catch (exception) {
...
const deleteData = async (req, res) => {
  try {
    const data = await Detail.findByIdAndUpdate(
      req.params._id,
      { $set: { status: "inactive" } },
      { new: true }
    ).populate("typeObjet_id contact_id");
...
const inactiveTypeObjet = async (_id) => {
  try {
    const data = await TypeObject.findByIdAndUpdate(
      _id,
      { $set: { status: "inactive" } },
      { new: true }
    );
